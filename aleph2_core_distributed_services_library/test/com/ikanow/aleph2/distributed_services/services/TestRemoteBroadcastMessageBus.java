...
public class TestRemoteBroadcastMessageBus {
	...
	public static class EmbeddedTestBean implements Serializable {
		private static final long serialVersionUID = -3145728917059789154L;
	...
	public void testRemoteBroadcast() throws IOException {
		...
		assertTrue("Initial check on message numbers (1):" + _received_bus1, _received_bus1 >= MESSAGES_TO_SEND);
		assertTrue("Initial check on message numbers (2):" + _received_post_bus2, _received_post_bus2 >= MESSAGES_TO_SEND);
		if (px.isAlive()) {
			px.destroyForcibly();			
		}
