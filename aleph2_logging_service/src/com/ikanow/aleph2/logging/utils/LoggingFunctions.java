...
public class LoggingFunctions {
	...
	public static BiFunction<BasicMessageBean, BasicMessageBean, BasicMessageBean> countMessages() {	
		return (n,o)->{
			...
			return BeanTemplateUtils.clone(n).with(BasicMessageBean::details, copyDetailsPutValue(n, COUNT_FIELD, count)).done();
		};
	}
	...
	public static BiFunction<BasicMessageBean, BasicMessageBean, BasicMessageBean> minField(final String field_to_min) { 
		return (n,o)->{	
			...
			return BeanTemplateUtils.clone(n).with(BasicMessageBean::details, copyDetailsPutValue(n, MIN_FIELD, Optional.ofNullable(getMinMaxOrNull(valOld, valNew, true)).orElse(null))).done();
		};
	}
	...
	public static BiFunction<BasicMessageBean, BasicMessageBean, BasicMessageBean> maxField(final String field_to_max) { 
		return (n,o)->{
			...
			//set min field to min or null if neither details had a value				
			return BeanTemplateUtils.clone(n).with(BasicMessageBean::details, copyDetailsPutValue(n, MAX_FIELD, Optional.ofNullable(getMinMaxOrNull(valOld, valNew, false)).orElse(null))).done();
		};
	}
	...
	public static Optional<Function<Tuple2<BasicMessageBean, Map<String, Object>>, Boolean>> logOutsideThreshold(final String value_field,
		...
		return Optional.of((t)->{
			...
			boolean result = min_threshold.map(m -> val < m).orElse(false) || 
					max_threshold.map(m -> val > m).orElse(false);
			if ( min_threshold.isPresent() && max_threshold.isPresent())
