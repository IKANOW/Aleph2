 ...
 */
public interface ICrudService<O> extends IDataWriteService<O> {
	...
	//FROM IBulkDataWriteService: storeObject			
	...
	//FROM IBulkDataWriteService: storeObjects		
	...
	//FROM IBulkDataWriteService: countObjects	
	...
	//FROM IBulkDataWriteService: deleteDatastore
	 ...
	 */
	ICrudService<JsonNode> getRawService();
	...
	/** Retrieves a batch write subservice if one exists (handling deduplication unlike its parent getBatchWriteSubservice)
	 * @return the batch write subservice
	 */
	default Optional<IBatchSubservice<O>> getBatchCrudSubservice() {
		return Optional.empty();
	}
	
	/** A more efficiently batch interface that may or may not be implemented - version supporting deduplication on insert
	 * @author Alex
	 */
	public static interface IBatchSubservice<O> extends IDataWriteService.IBatchSubservice<O> {
	...
	//FROM IBulkDataWriteService: getUnderlyingPlatformDriver
	
	/** Retrives the metamodel driver for this technology, if one exists
	 * @return
	 */
	default Optional<IMetaModel> getMetaModel() {
		return Optional.empty();
	}
